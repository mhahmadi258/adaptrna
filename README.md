...
python -m rinalmo_hub.cli.train    --task <task> --config configs/tasks/<task>.yaml [--use_lora] ...

# evaluate a trained artifact on the test or validation split
python -m rinalmo_hub.cli.evaluate --adapter <adapter.pt> --config configs/tasks/<task>.yaml
...
Create `rinalmo_hub/tasks/my_task.py`:
...
Then add one line to `rinalmo_hub/tasks/__init__.py`, so the decorator fires on import:
...
Create `configs/tasks/my_task.yaml`. Anything not set here is inherited from
`configs/base.yaml`:
...
```bash
python -m rinalmo_hub.cli.train --task my_task --config configs/tasks/my_task.yaml --use_lora \
...
```bash
# LoRA
python -m rinalmo_hub.cli.train --task splice_site --config configs/tasks/splice_site.yaml \
...
python -m rinalmo_hub.cli.evaluate --adapter outputs/splice_donor_lora/splice_site_adapter.pt \
    --config configs/tasks/splice_site.yaml --split test \
...
python -m rinalmo_hub.cli.train --task mrl --config configs/tasks/mrl.yaml \
    --set finetune.schedule=ft_schedules/mrl_paper.yaml --seed 42 --save_full_weights \
...
python -m rinalmo_hub.cli.evaluate --adapter outputs/mrl_lora/mrl_adapter.pt \
    --config configs/tasks/mrl.yaml --split test \
...
```bash
# LoRA on bpRNA
python -m rinalmo_hub.cli.train --task sec_struct --config configs/tasks/sec_struct.yaml \
    --use_lora --set optim.lr=3e-4 --seed 42 \
    --output_dir outputs/sec_struct_lora

# full fine-tuning with the gradual-unfreezing schedule
python -m rinalmo_hub.cli.train --task sec_struct --config configs/tasks/sec_struct.yaml \
    --set finetune.schedule=ft_schedules/sec_struct.yaml --seed 42 --save_full_weights \
...
python -m rinalmo_hub.cli.evaluate --adapter outputs/sec_struct_lora/sec_struct_adapter.pt \
    --config configs/tasks/sec_struct.yaml --split test \
...
python -m rinalmo_hub.cli.train --task ncrna_classification \
    --config examples/ncrna_classification/config.yaml --use_lora \
...
python -m rinalmo_hub.cli.evaluate --adapter adapters/<task>_adapter.pt \
    --config configs/tasks/<task>.yaml --split test
```
