...
# The source repo was split between the standalone `pytorch_lightning` package (this file and
# the splice-site training script) and the unified `lightning.pytorch` (everything else).
# `Trainer` rejects a datamodule from the other package with a bare "Expected a parent", so
# this framework standardises on `lightning.pytorch` everywhere.
